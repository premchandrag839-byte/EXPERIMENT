...
export default function HighSchool2024Page() {
  const [active, setActive] = useState(null);
  const TOPPERS_2024_HS = achievementsData[2024].highSchool;
  const router = useRouter();
  return (
    <div className="font-inter">
      <SEO title="High School Board Exam Toppers (2023–2024)" description="Top 5 high school board exam toppers for 2023–2024." />
      <Section title="High School Board Exam Toppers" subtitle="Batch 2023 – 2024">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 animate-fade-in">
          {TOPPERS_2024_HS.map((t, i) => (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-8 flex items-center gap-3">
        ...
        <button onClick={() => router.push('/achievements/2024/intermediate')} className="px-4 py-2 rounded bg-primary text-white hover:bg-primary-dark">Next</button>
      </div>
      <Footer />
